...
export default function ManageUsers() {
  ...
  const handleToggleActive = async (user) => {
    try {
      const res = await toggleUser(user.id, { active: !user.active })
  ...
  const handleToggleRole = async (user) => {
    ...
    try {
      const res = await toggleUser(user.id, { role: newRole })
      setUsers((prev) => prev.map((u) => u.id === user.id ? res.data : u))
      if (selectedUser?.id === user.id) setSelectedUser(res.data)
    } catch (err) {
      console.error(err)
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="bg-vault-dark border border-vault-green-dark rounded-lg overflow-hidden">
          <div className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-vault-green-dark">
            ...
            <span className="col-span-2 text-vault-hint text-xs tracking-widest uppercase">Rol</span>
          ...
          {filtered.map((user, i) => (
            ...
            >
              <div className="col-span-3 flex items-center gap-3">
                ...
                <span className="text-vault-text text-xs font-bold truncate">{user.name}</span>
              </div>
            ...
            <div className="bg-vault-dark border border-vault-green-dark rounded-lg p-6 w-full max-w-md font-mono">
              ...
              <div className="flex flex-col gap-3 mb-6">
                {[
                  ...
                  { label: 'Compras',  value: selectedUser.purchases_count || 0 },
                  { label: 'Gastado',  value: `${parseFloat(selectedUser.purchases_sum_price_paid || 0).toFixed(2)}€` },
                ].map((item) => (
